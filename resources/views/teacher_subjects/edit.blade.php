<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-semibold text-gray-800">✏️ Edit Teacher Subject</h2>
        ...
        <div class="w-full max-w-md">
            ...
            <div class="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden">
                ...
                <div class="px-6 py-4 text-center border-b border-gray-200 bg-blue-50">
                    <h3 class="text-lg font-medium text-blue-800">Edit Teacher Subject</h3>
                </div>

                <div class="px-6 py-6 space-y-4" x-data="teacherForm()">
                    <form action="{{ route('teacher_subjects.update', $teacherSubject->id) }}" method="POST" class="space-y-4">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <select name="class_id"
                                ...
                                @foreach($classes as $class)
                                    <option value="{{ $class->id }}" {{ $teacherSubject->class_id == $class->id ? 'selected' : '' }}>
                                        {{ $class->class_name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="section_id"
                                ...
                                @foreach($sections as $section)
                                    <option value="{{ $section->id }}" {{ $teacherSubject->section_id == $section->id ? 'selected' : '' }}>
    ...
    <script>
        function teacherForm() {
            return {
                ...
                teacher_name: '{{ $teacherSubject->teacher_name }}',
                updateTeacherName() {
