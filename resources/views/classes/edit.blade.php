<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-lg font-semibold text-gray-800">✏️ Edit Class</h2>
    ...
    <div class="min-h-[calc(100vh-4rem)] flex items-start justify-center bg-gray-50 py-10 px-4">
        <div class="w-full max-w-md">
            ...
            <div class="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden">
                ...
                <!-- Card Header -->
                <div class="px-6 py-4 text-center bg-green-100 border-b border-green-200">
                    <h3 class="text-lg font-semibold text-green-800">Edit Class</h3>
                </div>
                ...
                <div class="px-6 py-6 space-y-4">
                    <form action="{{ route('classes.update', $class->id) }}" method="POST" class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="number" name="class_numeric" 
                                   value="{{ old('class_numeric', $class->class_numeric) }}" 
                                   class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:ring-1 focus:ring-green-400 focus:outline-none">
                            @error('class_numeric')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
