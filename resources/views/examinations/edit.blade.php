<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-semibold text-gray-800">Edit Examination</h2>
        ...
        <div class="w-full max-w-md">
            ...
            <div class="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden">
                ...
                <div class="px-6 py-4 text-center border-b border-gray-200 bg-blue-50">
                    <h3 class="text-lg font-medium text-blue-800">Update Examination</h3>
                </div>

                <!-- Form -->
                <div class="px-6 py-6 space-y-4">
                    <form action="{{ route('examinations.update', $examination->id) }}" method="POST" class="space-y-4">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <input type="text" name="exam_name" value="{{ old('exam_name', $examination->exam_name) }}"
                        ...
                        <div>
                            ...
                            <select name="exam_type" 
                                    class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:ring-1 focus:ring-blue-400 focus:outline-none">
                                <option value="test" {{ $examination->exam_type === 'test' ? 'selected' : '' }}>Test</option>
                                <option value="half_yearly" {{ $examination->exam_type === 'half_yearly' ? 'selected' : '' }}>Half Yearly</option>
                                <option value="annual" {{ $examination->exam_type === 'annual' ? 'selected' : '' }}>Annual</option>
                            </select>
                            @error('exam_type')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <select name="academic_year_id" 
                                ...
                                @foreach($academicYears as $year)
                                    <option value="{{ $year->id }}" {{ $examination->academic_year_id == $year->id ? 'selected' : '' }}>
                                        {{ $year->year_name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="date" name="start_date" value="{{ old('start_date', $examination->start_date) }}"
                                   class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:ring-1 focus:ring-blue-400 focus:outline-none">
                            @error('start_date')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="date" name="end_date" value="{{ old('end_date', $examination->end_date) }}"
                                   class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:ring-1 focus:ring-blue-400 focus:outline-none">
                            @error('end_date')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <select name="status" 
                                    class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:ring-1 focus:ring-blue-400 focus:outline-none">
                                <option value="upcoming" {{ $examination->status === 'upcoming' ? 'selected' : '' }}>Upcoming</option>
                                <option value="ongoing" {{ $examination->status === 'ongoing' ? 'selected' : '' }}>Ongoing</option>
