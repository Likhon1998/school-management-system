        ...
        <div class="w-full max-w-lg">
            ...
            <div class="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden">
                ...
                <div class="px-6 py-4 text-center border-b border-gray-200 bg-blue-50">
                    <h3 class="text-lg font-medium text-blue-800">Edit Exam Schedule</h3>
                </div>

                <!-- Form -->
                <div class="px-6 py-6 space-y-4">
                    <form action="{{ route('exam_schedules.update', $examSchedule->id) }}" method="POST" class="space-y-4">
                        @csrf
                        @method('PUT')
                            ...
                            <select name="examination_id" 
                                ...
                                @foreach($examinations as $exam)
                                    <option value="{{ $exam->id }}" 
                                        {{ $examSchedule->examination_id == $exam->id ? 'selected' : '' }}>
                                        {{ $exam->exam_name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select name="subject_id" 
                                ...
                                @foreach($subjects as $subject)
                                    <option value="{{ $subject->id }}" 
                                        {{ $examSchedule->subject_id == $subject->id ? 'selected' : '' }}>
                            ...
                            <select name="class_id" 
                                ...
                                @foreach($classes as $class)
                                    <option value="{{ $class->id }}" 
                                        {{ $examSchedule->class_id == $class->id ? 'selected' : '' }}>
                                        {{ $class->class_name }}
                                    </option>
                                @endforeach
                            </select>
