<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-semibold text-gray-800">✏️ Edit Teacher</h2>
            ...
            <form action="{{ route('teachers.update', $teacher->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" name="salary" value="{{ old('salary', $teacher->salary) }}"
                               class="w-full border px-3 py-2 rounded focus:ring-2 focus:ring-blue-400 focus:outline-none">
                        @error('salary')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        @if($teacher->photo)
                            <img src="{{ asset('storage/' . $teacher->photo) }}" alt="Teacher Photo" class="mt-2 w-32 h-32 object-cover rounded border">
                        @endif
                    </div>
