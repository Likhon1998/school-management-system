<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-semibold text-gray-800">✏️ Edit Assigned Teacher</h2>
        ...
        <div class="w-full max-w-md">
            ...
            <div class="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden">
                ...
                <div class="px-6 py-4 text-center border-b border-gray-200 bg-blue-50">
                    <h3 class="text-lg font-medium text-blue-800">Edit Assigned Teacher</h3>
                </div>

                <!-- Form -->
                <div class="px-6 py-6 space-y-4" x-data="classTeacherForm()">
                    <form action="{{ route('class_teachers.update', $classTeacher->id) }}" method="POST" class="space-y-4">
                        @csrf
                        @method('PUT')
                            ...
                            <select name="teacher_id" x-model="teacher_id" @change="updateTeacherName"
                                ...
                                @foreach($teachers as $teacher)
                                    <option value="{{ $teacher->id }}" data-name="{{ $teacher->teacher_name }}" 
                                        {{ $classTeacher->teacher_id == $teacher->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select name="class_id"
                                ...
                                @foreach($classes as $class)
                                    <option value="{{ $class->id }}" {{ $classTeacher->class_id == $class->id ? 'selected' : '' }}>
                                        {{ $class->class_name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="section_id"
                                ...
                                @foreach($sections as $section)
                                    <option value="{{ $section->id }}" {{ $classTeacher->section_id == $section->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select name="academic_year_id"
                                ...
                                @foreach($academicYears as $year)
                                    <option value="{{ $year->id }}" {{ $classTeacher->academic_year_id == $year->id ? 'selected' : '' }}>
                                        {{ $year->year_name }}
                                    </option>
                                @endforeach
                            </select>
    ...
    <script>
        function classTeacherForm() {
            return {
                ...
                teacher_name: '{{ $classTeacher->teacher_name }}',
                updateTeacherName() {
